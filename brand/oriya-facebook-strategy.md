# STRATÉGIE FACEBOOK ÔRIYA MAROC
## Bijoux Plaqués Or - Maroc

---

## 1. CONFIGURATION PAGE FACEBOOK

### Informations Essentielles
- **Nom de la page** : Ôriya Maroc
- **Catégorie** : Bijouterie / Boutique de Bijoux
- **Région** : Maroc
- **Langue** : Français / Darija

### Description Courte (160 caractères)
```
✨ Bijoux plaqués or tendance pour les femmes marocaines modernes.
Qualité premium, prix accessibles. Commande facile via WhatsApp.
```

### Description Longue (200+ mots)
```
Bienvenue chez Ôriya Maroc !

Nous créons des bijoux plaqués or qui racontent votre histoire.
Chaque pièce est pensée pour sublimer votre féminité, que vous soyez
mère de famille, professionnelle moderne ou jeune femme en quête
d'authenticité.

Chez Ôriya, on croit que le luxe n'est pas une question de prix,
mais de feeling. Nos bijoux allient :

✨ Qualité premium : Plaqué or véritable, résiste au temps
💎 Designs tendance : Inspirés des plus belles maisons de joaillerie
🤝 Relation humaine : Conseil, écoute, suivi personnel
⚡ Accessibilité : Des prix justes pour toutes les femmes marocaines

Que ce soit pour un mariage, une occasion spéciale ou simplement
pour vous sentir belle au quotidien, Ôriya est là pour vous.

Découvrez nos collections de bagues, colliers, bracelets et
boucles d'oreilles. Chaque achat est une pièce d'or, une confiance,
un bijou qui devient votre compagnon.

Commander c'est facile : un message WhatsApp, et nous nous occupons
du reste. Paiement flexible, livraison discrète, conseil gratuit.

Ôriya, c'est l'or qui vous ressemble.
```

### Call-to-Action Principal
- **Bouton CTA** : "Commande sur WhatsApp"
- **Lien** : https://wa.me/212XXXXXXXXX (à compléter avec le numéro Ôriya)

### Website
```
URL Principal : https://issam0505-oriya-maroc-dashboard.hf.space
```

---

## 2. IMAGES DE COUVERTURE RECOMMENDATIONS

### VARIATION 1 : "Luxe & Élégance" (16:9)
**Dimensions** : 1200 x 675 pixels (optimisé pour desktop & mobile)

**Contenu** :
- Fond : Dégradé noir/charbon avec textures subtiles (marbre ou velours)
- Élément principal : 3-4 bijoux Or en disposition asymétrique (bague, collier, bracelet)
- Accent : Touches de cream/beige pour contraste
- Texte overlay (optionnel) : "Ôriya Maroc ✨" en Or (font élégante, serif)
- Éléments secondaires : Feuilles botaniques discrètes, fleurs séchées

**Palette de couleurs** :
- Fond : #1a1a1a (noir luxe)
- Or : #d4af37 ou #c9a961 (or plaqué)
- Accent : #f5f1e8 (cream)
- Texte : Blanc ou Or

**Conseil** : Éviter la surcharge. Minimalisme luxe.

---

### VARIATION 2 : "Jeunesse & Dynamisme" (1:1)
**Dimensions** : 1080 x 1080 pixels (optimisé pour story & carousel)

**Contenu** :
- Fond : Blanc ou cream très clair avec texture subtile
- Élément principal : 1 bijou statement (gros plan, haute définition) - ex: bague imposante
- Accent : Ombre légère pour profondeur
- Texte : "Ôriya Maroc" avec emoji (✨ ou 💎)
- Éléments secondaires : Géométrie minimaliste (lignes, points dorés)

**Palette de couleurs** :
- Fond : #f9f7f2 (cream pale)
- Or : #d4af37
- Texte : #2a2a2a (noir doux)
- Détails : Touches de rose poudré (#e8c4d0) pour modernité

**Conseil** : Parfait pour les stories et les pubs Instagram reprises sur Facebook.

---

## 3. CONTENU 10 PREMIERS POSTS (2 SEMAINES)

### JOUR 1 - DIMANCHE | TYPE : Welcome Post
**Format** : Image + Carousel (3 slides)
**Caption** :
```
Salaam Ôriya Maroc ! 🌙✨

Nous, c'est pas juste des bijoux... c'est une relation.

Chaque femme marocaine mérite de se sentir magnifique. Pas besoin
de se ruiner, pas besoin de faire du compromis sur la qualité.

Chez Ôriya, on a compris que vous vouliez des bijoux plaqués or
qui durent, qui brillent, qui vous racontent.

Bienvenue à notre table. ✨
```
**Hashtags** : #ÔriyaMaroc #BijouxOr #QueensMorocco #LuxeAccessible #BijouxTendance
**CTA** : "Découvre nos collections" (bouton Facebook ou lien WhatsApp)
**Format image** :
- Slide 1: Hero image couverture
- Slide 2: Close-up bijoux
- Slide 3: Témoignage client souriant

---

### JOUR 2 - LUNDI | TYPE : Educational Post
**Format** : Carrousel (4 slides)
**Caption** :
```
Les 4 packs Ôriya pour chaque moment de ta vie 💎

Pack Solo : Ton bijou statement qui parle pour toi
Pack Couple : À partager avec celle qui te comprend
Pack Trio : L'art du layering maîtrisé
Pack Golden : Ton secret pour briller tous les jours

Laquelle tu choisirais ? ↓
```
**Hashtags** : #BijouxPacks #CombosÔriya #LayeringGame #BijouxOr
**CTA** : "En savoir plus sur les packs"
**Format image** : Carrousel avec pack par slide, prix discret en bas

---

### JOUR 3 - MARDI | TYPE : Testimonial Post
**Format** : Image + texte
**Caption** :
```
"Ôriya, c'est mes bijoux de tous les jours."

Aida, 34 ans, Marrakech.

Elle nous a dit : "J'ai acheté une bague il y a 6 mois. Elle brille
toujours comme le premier jour. Et le prix ? Accessible sans regret."

C'est ça, Ôriya. Des bijoux qui vous suivent, pas juste qui vous
plaisent un jour.

Votre avis compte. Partagez votre expérience Ôriya en commentaire 👇
```
**Hashtags** : #AvisClients #ÔriyaMaroc #QualitéOr #Authenticité
**CTA** : "Partage ton avis Ôriya"
**Format image** : Photo cliente avec son bijou, style candid (pas trop posé)

---

### JOUR 4 - MERCREDI | TYPE : Style Post / Inspiration
**Format** : Vidéo ou Carrousel (5 slides)
**Caption** :
```
L'art du layering selon Ôriya ✨

Fine + Statement = Perfection

Slide 1 : Fine chaîne discrète
Slide 2 : + Bague classique
Slide 3 : + Bracelet imposant
Slide 4 : L'ensemble complet (photo finale)
Slide 5 : "Ton tour ! Montre-nous ton layering"

Combien de pièces Ôriya as-tu déjà empilées ?
```
**Hashtags** : #LayeringGame #BijouxTendance #StylingInspo #BijouxOr
**CTA** : "Compose ton combo Ôriya"
**Format image** : Carrousel step-by-step ou vidéo 30 secondes

---

### JOUR 5 - JEUDI | TYPE : Offer Post
**Format** : Image
**Caption** :
```
🎁 SURPRISE ÔRIYA JEUDI !

Valable jusqu'à demain minuit :

10% de réduction sur ta 2ème pièce Ôriya
(Code : ÔRIYA10 - WhatsApp seulement)

Oui, on veut que tu trouves ta paire. Parce qu'une seule bague,
ça peut pas suffire. 😊

Qui est avec nous ? 👇
```
**Hashtags** : #ÔriyaPromo #JeudiRéduction #BijouxMaroc #OffreSpéciale
**CTA** : "Commande sur WhatsApp"
**Format image** : Image simple avec code promo en évidence, fond or/noir

---

### JOUR 6 - VENDREDI | TYPE : Community Post
**Format** : Text + Image
**Caption** :
```
Vendredi, c'est le jour où on se pose la vraie question...

Tu préfères les bagues fines et discrètes ou les pièces statement
qui parlent pour toi ? 💎

Nous, chez Ôriya, on dit que tu as raison dans les deux cas.
C'est toi qui décides qui tu es ce jour-là.

Dis-nous en commentaire ! ↓
```
**Hashtags** : #BijouxPersonnalité #VendrediQuestion #BijouxOr
**CTA** : Aucun (juste engagement)
**Format image** : Collage ou flatlay : 1 bague fine + 1 bague statement

---

### JOUR 7 - SAMEDI | TYPE : Behind-the-Scenes
**Format** : Vidéo courte ou Carrousel (4 slides)
**Caption** :
```
Coulisses Ôriya 🎬✨

Comment on sélectionne l'or de tes bijoux ?
Comment on teste la qualité ?
Comment on s'assure que tu reçois une pièce parfaite ?

Spoiler : On est pointilleuses. 💪

Regarde derrière le rideau et comprends pourquoi Ôriya,
c'est pas juste beau... c'est SOLIDE.
```
**Hashtags** : #BisooBTS #QualitéOr #TransparentÔriya #CraftsmanshipMatters
**CTA** : "Apprends comment on fonctionne"
**Format image** : Vidéo 30-45 sec ou carrousel montrant processus (discrétion)

---

### JOUR 8 - DIMANCHE | TYPE : Inspirational Post
**Format** : Image + Texte
**Caption** :
```
"Je ne dépense pas. J'investis."

Les femmes marocaines ont compris quelque chose :
la vraie beauté, ça dure.

Un bijou Ôriya, ce n'est pas une dépense d'impulse.
C'est une pièce que tu vas garder, que tu vas offrir,
que tu vas raconter.

C'est ça, le luxe marocain. Pas bruyant.
Juste présent. Juste toi.

Es-tu avec nous ? ✨
```
**Hashtags** : #LuxeAccessible #InvestmentInBeauty #BijouxMaroc #Féminité
**CTA** : "Trouve ta pièce Ôriya"
**Format image** : Photo lifestyle : femme mature portant bijou, naturelle, confiante

---

### JOUR 9 - LUNDI | TYPE : Product Showcase
**Format** : Carrousel (5 slides)
**Caption** :
```
Coup de cœur collection JUIN ✨

Slide 1 : La BAGUE ÉTOILE - Minimaliste, intemporelle
Slide 2 : Le COLLIER LUNE - Poétique, féminin
Slide 3 : Le BRACELET CHAÎNE - Discret mais présent
Slide 4 : Les BOUCLES GÉOMÉTRIQUES - Modernes, affirmées
Slide 5 : "Laquelle est ta préférée ?"

Chaque pièce raconte une histoire. Quelle est la tienne ? 👇
```
**Hashtags** : #CollectionÔriya #BijouxTendance #CoupDeCoeur #Design
**CTA** : "Découvre les détails"
**Format image** : Haute résolution, fond blanc/cream, shadow pour profondeur

---

### JOUR 10 - MARDI | TYPE : Call-to-Action Direct
**Format** : Image + Texte
**Caption** :
```
Prête pour ta première Ôriya ? 💎

On te l'avoue : les meilleures décisions se prennent
en 5 minutes avec quelqu'un de confiance.

Ôriya n'est jamais loin. Un message WhatsApp,
et on devient tes conseillères bijoux.

Styles ? Budget ? Occasion ? On te guide.
Aucune pression. Juste du conseil honnête.

Envoie-nous un message. On t'attend. ✨
```
**Hashtags** : #ÔriyaMaroc #ConseilleBijoux #MessageNous #CommenceAujourdhui
**CTA** : "Envoie un message WhatsApp"
**Format image** : Photo d'un téléphone montrant conversation WhatsApp, chaleureuse

---

## 4. CAROUSEL POSTS (3 EXEMPLES DÉTAILLÉS)

### CAROUSEL POST 1 : "Nos 4 Packs Ôriya"

**Titre du post** :
```
Les 4 packs Ôriya : Trouve le tien ! 💎

Chaque femme est différente. Tes bijoux aussi.
Quel pack raconte ton histoire ?
```

**Slide 1 - Pack SOLO**
```
🎯 PACK SOLO : Le Bijou Qui te Suffit

Parfait pour : Les femmes qui aiment la simplicité
             Les minimalistes chic
             Celles qui veulent UN bijou qui parle

- 1 Bague ou 1 Collier STATEMENT
- Qualité premium plaqué or
- Boîtier Ôriya inclus
- Garantie 6 mois

Prix : 249-349 MAD

"Un bijou. Une femme. Une histoire."
```

**Slide 2 - Pack COUPLE**
```
💕 PACK COUPLE : À Partager

Parfait pour : Les meilleures amies
             Mère & Fille
             Sœurs complices
             Ton amoureux & toi

- 2 Pièces assorties ou complémentaires
- Plaqué or véritable
- 2 Boîtiers Ôriya (cadeaux)
- Livraison ensemble

Prix : 449-599 MAD

"Deux femmes, deux styles. Une connexion."
```

**Slide 3 - Pack TRIO**
```
✨ PACK TRIO : Le Layering Maîtrisé

Parfait pour : Celles qui aiment l'art du layering
             Les femmes qui veulent la flexibilité
             Les occasions spéciales
             Chaque jour de la semaine

- 3 Pièces : Fine chaîne + Bague classique + Bracelet
- Portables seules ou ensemble
- Boîtier spécial rangement Ôriya
- Carnet de styling inclus

Prix : 699-899 MAD

"L'harmonie en trois pièces."
```

**Slide 4 - Pack GOLDEN (VIP)**
```
👑 PACK GOLDEN : L'Expérience Complète

Parfait pour : Les vraies amoureux de Ôriya
             Les femmes qui veulent tout
             Les grandes occasions
             S'offrir le meilleur

- 5 Pièces sélectionnées avec soin
- Consultation styliste GRATUITE (WhatsApp)
- Boîtier de luxe édition limitée
- Certificat d'authenticité or
- Accès prioritaire nouvelles collections

Prix : 1299-1699 MAD

"Tu mérites les étoiles. C'est ton pack."
```

**CTA Final** :
```
Laquelle tu crois qui te ressemble le plus ?
Envoie-nous un message pour commencer ! 💌
```

---

### CAROUSEL POST 2 : "Comment Layerer tes Bijoux"

**Titre du post** :
```
Le Guide Complet du Layering selon Ôriya ✨

Tu veux ressembler aux filles qui savent ?
On te montre comment faire.
```

**Slide 1 - Les Basiques**
```
🎓 LESSON 1 : LES BASIQUES DU LAYERING

Le layering, c'est l'art de combiner plusieurs pièces
sans faire du "trop".

3 Règles d'Or :
1️⃣  Mélange les épaisseurs (fine + medium + statement)
2️⃣  Assortis les métal ou accepte la mix
3️⃣  Moins est plus (même avec beaucoup)

Commençons doucement... ↓
```

**Slide 2 - La Combo Fine**
```
💫 NIVEAU 1 : LA COMBO FINE

Parfait pour tous les jours.

Tu auras besoin :
- 1 Fine chaîne or (collier)
- 1 Bague fine classique
- 1 Bracelet discret

Comment combiner :
Chaîne courte au cou + Bague moyen doigt + Bracelet poignet

Style : Discret mais présent. Chic. Féminin.

C'est déjà beautiful 🤎
```

**Slide 3 - L'Évolution Moyenne**
```
✨ NIVEAU 2 : L'ÉVOLUTION

Prêt(e) pour plus d'impact ?

Tu ajoutes :
- 1 Bague medium/imposante
- 1 Collier moyen

Comment combiner :
Fine chaîne + COLLIER + 2 Bagues différentes + Bracelet

Style : On te voit. Tu es présente. Féminité affirmée.

(Ça commence à parler, non ?) 💎
```

**Slide 4 - Full Layering (L'Apogée)**
```
👑 NIVEAU 3 : FULL LAYERING

Tu domines. C'est assumé.

Tu crées :
- 2-3 Chaînes de différentes longueurs (OU 1 collar + 1 fine)
- 2-3 Bagues variées
- 1-2 Bracelets
- Boucles d'oreilles statement (optionnel)

Comment combiner :
Tout ensemble. Harmonieusement. Avec confiance.

Style : C'est ton uniforme de reine. 👑

Secret : Même les femmes de Fès et Casablanca qui portent ça
disent "Je n'ai pas fait exprès" 😏
```

**Slide 5 - L'Astuce Ôriya**
```
🎁 L'ASTUCE ÔRIYA

Comment tu sais si c'est réussi ?

✅ Ça te plaît avant tout
✅ Ça va avec ce que tu portes
✅ Ça raconte quelque chose sur toi
✅ Personne ne peut le contredire

Règle d'or : Le layering c'est pas ce que les autres pensent.
C'est ce que TU ressens.

Ôriya te donne les outils.
Ton style, c'est le tien.

Et si tu veux de l'aide ? On est là. 💌
```

**CTA Final** :
```
Teste ce weekend et montre-nous ton layering en commentaire !
#ÔriyaLayering
```

---

### CAROUSEL POST 3 : "Avis Clients Authentiques"

**Titre du post** :
```
Les Vraies Ôriya Girls Te Disent Pourquoi ✨

Des témoignages sincères. Pas de pub. Juste de la vérité.
```

**Slide 1 - Testimonial FATIMA**
```
⭐⭐⭐⭐⭐

"Je recherchais quelque chose d'authentique depuis des années."

FATIMA, 38 ans - Rabat

"Ôriya m'a trouvé. Les bijoux d'or plaqué que j'avais avant,
ils noircissaient après 2 mois. Là, 8 mois et c'est parfait.

Ma fille m'a même demandé d'où ça vient.
Ça me plaît que mes bijoux durent et que j'aime vraiment les porter."

Merci Fatima. ❤️
```

**Slide 2 - Testimonial LEILA**
```
⭐⭐⭐⭐⭐

"Ôriya, c'est du conseil. Pas juste de la vente."

LEILA, 26 ans - Marrakech

"J'ai commandé une bague et j'étais stressée : et si ça me plaît pas ?

Mais Ôriya m'a appelée, on a discuté 20 minutes sur les styles,
mes mains, mes préférences. Elle a compris avant moi.

La bague est arrivée parfaite. Depuis j'ai recommandé à 3 copines.
C'est ça qui en parle le plus."

Merci Leila. Vous êtes notre fierté. ❤️
```

**Slide 3 - Testimonial AMINA**
```
⭐⭐⭐⭐⭐

"C'est pas juste des bijoux. C'est une relation."

AMINA, 45 ans - Fès

"Après un divorce difficile, je me suis sentie invisible.
J'ai regardé les bijoux Ôriya un soir et je me suis dit 'pourquoi pas'.

J'ai commandé. Et en recevant, c'était comme un clin d'œil
du monde qui disait 'tu es belle, tu existes, tu comptes'.

Maintenant je porte cette bague tous les jours.
Elle me rappelle que je suis à moi."

Merci Amina. Pour ton courage et ta confiance. ❤️
```

**Slide 4 - Testimonial YASMINA**
```
⭐⭐⭐⭐⭐

"Qualité + Prix accessible = Ôriya"

YASMINA, 24 ans - Casablanca

"Je suis étudiante et je voulais des bijoux or sans me ruiner.
Ôriya c'est l'équilibre parfait.

J'ai eu 3 pièces pour moins que 1 bague chez les joailliers.
Et la qualité ? Franchement j'en reviens pas.

Je conseille à toutes mes copines maintenant.
C'est notre secret Millennial. 😊"

Merci Yasmina. À toi et toute ta génération. ❤️
```

**Slide 5 - Appel à Témoignage**
```
📢 C'EST TON TOUR !

Si tu as un avis Ôriya, on veut l'entendre.

Pas besoin d'être une influenceur.
Pas besoin d'être parfaite.
Juste d'être toi.

Comment tu trouves tes bijoux Ôriya ?
Comment ça a changé quelque chose dans ta vie ?

Envoie-nous un message WhatsApp ou commente ici.
On sera honorer de te partager.

#ÔriyaGirls #AvisAuthentique #ConfianceTotal
```

**CTA Final** :
```
Lire tous les avis Ôriya : [lien vers page avis]
Tu as une histoire à partager ? Contacte-nous 💌
```

---

## 5. VIDEO STRATEGY (3 IDÉES COURTES 30-60 SEC)

### VIDEO 1 : UNBOXING ÔRIYA

**Durée** : 45 secondes
**Style** : Intimate, ASMR vibe, warm

**Script/Plan** :
```
[0-5 sec] OPEN
- Son d'enveloppe qui s'ouvre (ASMR)
- Close-up des mains féminines ouvrant un colis Ôriya
- Text overlay : "L'expérience Ôriya commence ici"

[5-15 sec] UNBOXING
- Ouverture du boîtier Ôriya (élégant, minimaliste)
- Révélation du bijou or, lumière qui rebondit
- Close-up du bijou : "Regardez ce détail..."

[15-25 sec] DETAILS
- Certificat d'authenticité montré
- Note personnelle dans le colis
- Emballage recyclable/élégant

[25-35 sec] FEELING
- Portrait de femme qui découvre, sourire naturel
- Elle essaie le bijou
- Text : "Ça c'est Ôriya."

[35-45 sec] CTA
- Shot du colis complètement déballé
- Text overlay : "Commande maintenant sur WhatsApp"
- Logo Ôriya final + lien WhatsApp

VOIX : Ton warm, féminin, accent marocain léger
MUSIQUE : Ambient doux, luxueux (pas loud)
COULEURS : Or, cream, noir - cohérent avec identité
```

**Placements** :
- Facebook Feed
- Facebook Reels
- Instagram Stories (version courte 15sec)

---

### VIDEO 2 : TESTIMONIAL CLIENT

**Durée** : 50 secondes
**Style** : Documentary style, authentique, naturel

**Script/Plan** :
```
[0-5 sec] INTRO
- Woman in her home, natural light
- Text : "Salaam, je suis SARA"

[5-15 sec] QUESTION
- Voice over (Ôriya) : "Pourquoi Ôriya ?"
- Sara parle naturellement (pas scripted, c'est mieux)
- Exemple : "Parce que je voulais quelque chose de beau sans
  me ruiner. Et parce que ça dure vraiment."

[15-30 sec] DEMONSTRATION
- Elle montre son bijou (bague, collier, etc.)
- Close-ups du bijou
- Elle l'essaie devant caméra (naturel)
- Sourire authentique

[30-40 sec] IMPACT STORY
- She talks about how it makes her feel
- "C'est bizarre, mais c'est comme si ce bijou me donne confiance."
- Maybe scenes of her wearing it in daily life (work, family moment)

[40-50 sec] RECOMMENDATION
- "Je conseille Ôriya à mes amies sans hésiter"
- End shot: Wide smile to camera
- Text overlay: "Join thousands of Ôriya girls" + WhatsApp link

VOIX : Real testimonial (no professional voice-over)
MUSIQUE : Soft, background, subtle
LIGHTING : Natural light (home/outdoor preferred)
AUTHENTICITY : Key! No over-production
```

**Placements** :
- Facebook Feed (main format)
- Facebook Stories (loop 6-8 times)
- Evergreen content (relancer régulièrement)

---

### VIDEO 3 : PRODUCT SHOWCASE "THE DETAIL"

**Durée** : 45 secondes
**Style** : Cinematic, luxury, high-quality macro

**Script/Plan** :
```
[0-5 sec] HOOK
- Macro shot of gold jewelry (extreme close-up)
- Text : "Regardez où se trouve la magie..."

[5-15 sec] MACRO DETAILS
- Extreme close-up of gold plating
- Show craftsmanship details (engravings, polish, texture)
- Light reflections on gold surface
- Text overlay : "Plaqué or véritable. Qualité premium."

[15-25 sec] 360 ROTATION
- Product rotates on black/cream background
- Shows all angles
- Lighting changes subtly to highlight craftsmanship
- Music builds slightly

[25-35 sec] WEAR MOMENT
- Woman putting on the jewelry (hands only, or full body)
- Slow-motion shot of jewelry catching light
- Her confidence grows with each frame
- Text : "C'est ce qui se voit. Et ce qui compte vraiment."

[35-45 sec] CALL TO ACTION
- Final product shot on elegant background
- Text overlay : "Décvrez tous les détails Ôriya"
- WhatsApp button/link prominent
- Ôriya logo + tagline : "L'or qui vous ressemble"

VOIX : No voice-over (only music + ambient sound)
MUSIQUE : Luxury, cinematic, slightly dramatic
FILMING : Need macro lens, professional lighting, tripod
COLORS : Gold, black, cream - consistent with brand
```

**Production Notes** :
- Si tu n'as pas équipement pro, engage un videographe local
- Budget approximatif : 500-1000 MAD par vidéo (professionnel)
- Alternative DIY : smartphone + macro lens + good lighting

**Placements** :
- Facebook Feed (hero content)
- Facebook Video Ads
- Website/Dashboard homepage

---

## 6. FACEBOOK ADS BRIEF

### AUDIENCE TARGET (LOOKALIKE + CUSTOM AUDIENCE)

**Primary Audience** :
- **Femmes** : 25-50 ans
- **Localisation** : Maroc (tous les villes)
- **Langue** : Français, Darija
- **Intérêts** :
  - Bijoux & Fashion
  - Luxe & Accessoires
  - Beauté & Wellness
  - Mariage & Événements
  - Entrepreneuriat féminin
- **Comportements** :
  - Achats en ligne réguliers
  - Engagement produits de luxe
  - Actives sur Facebook/WhatsApp

**Secondary Audience** :
- **Jeunes femmes** : 18-28 ans
- **Mêmes localisation & intérêts**
- **+ Tendance** : Retail, Mode rapide, Influenceurs

**Excluded Audiences** :
- Personnes ayant déjà achetées (retargeting différent)
- Audiences trop larges (optimiser budget)

**Lookalike Audience** :
- Basée sur : Clients Ôriya actuels (1% lookalike initialement)
- À élargir : 1% → 3% → 5% au fur et à mesure

---

### BUDGET RECOMMANDÉ

**Scénario 1 : Budget Minimum (Test)**
```
Budget quotidien : 200 MAD/jour
Budget total : 1400 MAD/semaine
Durée : 2 semaines (2800 MAD total)
Objectif : Tester audiences & creative
ROI cible : 3:1 minimum
```

**Scénario 2 : Budget Standard (Recommandé)**
```
Budget quotidien : 350 MAD/jour
Budget total : 2450 MAD/semaine
Durée : 3-4 semaines (7350-9800 MAD)
Objectif : Scale proven winners
ROI cible : 4:1 minimum
```

**Scénario 3 : Budget Premium (Growth)**
```
Budget quotidien : 500 MAD/jour
Budget total : 3500 MAD/semaine
Durée : 4 semaines continu
Objectif : Dominant market presence
ROI cible : 4:1 minimum
```

**Distribution Budget** :
```
- 50% : Traffic campaigns (awareness + website visits)
- 30% : Conversion campaigns (direct sales)
- 20% : Retargeting campaigns (warm audiences)
```

---

### OBJECTIFS CAMPAIGN

**Campaign 1 : TRAFFIC (Awareness)**
```
Objectif Facebook : Traffic
Format : Carousel + Single image
Destination : Website dashboard
Durée : Ongoing (permanent awareness)

Message : "Découvre la collection Ôriya"
CTA : "En savoir plus"
Expected CTR : 2-4%
Expected CPM : 8-15 MAD
```

**Campaign 2 : CONVERSIONS (Sales)**
```
Objectif Facebook : Conversions
Format : Carousel + Video (unboxing)
Destination : WhatsApp click (avec script)
Durée : 2-3 weeks (promotional)

Message : "Commande ton first Ôriya"
CTA : "Commander maintenant"
Expected Conversion Rate : 1-2%
Expected CPC : 12-20 MAD
```

**Campaign 3 : ENGAGEMENT (Community)**
```
Objectif Facebook : Engagement
Format : Carousel testimonials + Questions
Destination : Facebook Post (comments)
Durée : Ongoing (weekly)

Message : "Partage ton avis Ôriya"
CTA : "Commente ton expérience"
Expected Engagement Rate : 3-6%
Expected Cost per Engagement : 0.5-1 MAD
```

---

### DURATION & SEASONALITY

**Phase 1 : Lancement (Semaines 1-2)**
- Budget : 200-300 MAD/jour
- Focus : Traffic + Brand Awareness
- Creative : Hero image + Testimonials
- Goal : 5000+ impressions, 100+ clicks

**Phase 2 : Optimization (Semaines 3-4)**
- Budget : 300-400 MAD/jour (augmenter les winners)
- Focus : Conversions + Retargeting
- Creative : Videos + Carousels
- Goal : 10+ conversions, 500+ website visits

**Phase 3 : Scale (Semaines 5+)**
- Budget : 400-500 MAD/jour
- Focus : Maintain, optimize, expand audiences
- Creative : Fresh content + New testimonials
- Goal : 20+ conversions/week, 200+ WhatsApp inquiries

**Seasonal Boosts** :
```
Février-Mars : Valentine season (couples jewelry)
Mai-Juin : Wedding season (statement pieces)
Juillet-Août : Summer collection launch
Septembre : Back to school / Back to work
Octobre-Novembre : Holiday gifts prep
Décembre : Year-end gift campaign
```

---

### CTA BUTTON & LANDING

**Primary CTA Button** : "Shop Now" ou "Commande Maintenant"
**Secondary CTA** : "Learn More" ou "Découvre"
**Custom CTA** : "Message WhatsApp" (si disponible dans région)

**Landing Page Options** :
1. Website dashboard (https://issam0505-oriya-maroc-dashboard.hf.space)
2. Facebook Catalog (product feed)
3. WhatsApp direct (best conversion)
4. Landing page customized (if budget allows)

**WhatsApp Integration** :
```
- Click button → WhatsApp message ouvre
- Pre-filled message : "Salaam Ôriya! Je suis intéressée par..."
- Phone link : https://wa.me/212XXXXXXXXX?text=Salaam%20Ôriya
- Set expectations : "Nous répondons en moins de 2h"
```

---

## 7. COMMUNITY MANAGEMENT

### HORAIRES DE RÉPONSE

**Optimal Response Hours** :
```
MATIN :
- Lun-Ven : 9h00-10h30 (avant travail, clients connectés)
- Sam : 10h00-11h00
- Dim : 10h00-11h00

MIDI :
- Lun-Ven : 12h00-13h00 (lunch break)
- Sam-Dim : 12h00-14h00

SOIR :
- Lun-Ven : 18h00-20h00 (après travail, moment prime)
- Sam-Dim : 18h00-21h00

NIGHT (si possible) :
- Lun-Ven : 20h00-22h00 (dernier check avant dodo)
- Sam-Dim : 20h00-23h00
```

**Response Time Target** :
```
- Comments : Réponse dans 30 min - 2 heures
- DM/WhatsApp : Réponse dans 15 min - 1 heure
- Urgent (client stressé) : ASAP, max 30 min
- Questions produit : 1-2 heures (permet research)
```

**Off-Hours Message** :
```
Si tu réponds après heures : "Merci pour ta question!
On te répond demain matin à la première heure. 💌"
```

---

### TON DE RÉPONSE (VOICE & TONE)

**Principes** :
1. **Chaleur** : Comme une amie, pas une machine
2. **Respect** : Chaque question mérite une vraie réponse
3. **Authenticité** : Pas de marketing lourd
4. **Professionnel + Personnel** : Balance parfaite
5. **Darija-friendly** : Si client écrit en darija, réponds en darija

**Exemples de Tons** :

Pour question simple (produit) :
```
"Salaam Layla! 💎

La bague ÉTOILE existe en 2 tailles : 50mm et 55mm.
C'est laquelle qui t'intéresse ?

(Et pour info, c'est un best-seller! 😊)"
```

Pour question stressée (délai, problème) :
```
"Salaam Fatima,

Je comprends ton stress. Les délais de livraison Maroc
sont 2-3 jours ouvrables. Si tu as commandé lundi,
tu l'auras mercredi/jeudi maximum.

Si tu ne l'as pas reçu jeudi, contacte-moi directement.
On va régler. Promise. 💪"
```

Pour compliment/testimonial :
```
"Oh Yasmine!! 😭💕

Ça nous fait TELLEMENT plaisir à lire!
Merci de nous faire confiance et d'avoir partagé
ta belle expérience.

Ôriyas très forts 💋✨"
```

Pour critique/retour négatif :
```
"Salaam Sara,

On est désolée que [X] n'a pas été à la hauteur.
C'est franchement pas notre style et on veux pas ça.

Envoie-moi un message privé. On va trouver une solution.
(Et sans condition, tu peux nous retourner.) 💪"
```

---

### FAQ PRÉPARÉES (5 QUESTIONS FRÉQUENTES)

**Q1: C'est du vrai or ?**
```
"Salaam! Oui, 100% or plaqué. Pas du doré qui s'enlève.

Voici la différence :
- Or plaqué ÔRIYA : Couche or véritable (2-3 microns)
  sur support metal hypoallergénique
- Doré cheap : Peinture or qu'on gratte

Garantie ÔRIYA : 6 mois. Si ça s'use, on remplace.
Aucun bijou Ôriya ne s'enlève avant 8-10 mois d'utilisation."
```

**Q2: Ça convient pour les peaux sensibles ?**
```
"Oui! Ôriya c'est pensé pour ça.

Support metal : Acier inoxydable hypoallergénique
(pas nickel, pas cadmium, rien de louche)

Recommendation : Si tu as la peau très sensible,
contacte-nous directement. On va te conseiller
les pièces les mieux adaptées."
```

**Q3: Combien de temps pour la livraison ?**
```
"Délais Ôriya Maroc :

Casablanca/Rabat : 24-48h
Autres villes : 2-3 jours ouvrables
Zones éloignées : 3-5 jours max

Bonus : On te notifie à chaque étape (expédition,
en route, livré).

Frais port : Gratuit à partir de 500 MAD
(sinon 30-50 MAD selon zone)"
```

**Q4: Vous acceptez les retours/échanges ?**
```
"Bien sûr!

Délai retour : 14 jours après réception
Raison : Pas besoin de justifier. Tu aimes pas? Retour.
Frais retour : On les paie (on envoie le label)
Remboursement : 7 jours après qu'on reçoive

Conditions :
- Bijou non porté / pas d'usure
- Avec boîtier Ôriya original
- Pas scratché ou endommagé

On cherche pas les petits détails. On comprend."
```

**Q5: Est-ce que vous livrez à l'international ?**
```
"Pas encore! On est 100% focus Maroc pour le moment.

Mais si tu es une Marocaine à l'étranger, envoie-moi
un message. On peut trouver une solution selon ta localisation.

Promesse : Bientôt on sera partout.
Pour l'instant, tu nous demandes. ❤️"
```

---

### SCRIPT STANDARD POUR ACCUEILLE NOUVELLE VISITEUR

**Quand quelqu'un commente 1ere fois** :
```
"Salaam [Prénom]! 💎

Bienvenue chez Ôriya!

Si tu as une question sur les bijoux, les tailles,
les couleurs, ou juste pour papoter... je suis là.

Excited pour te découvrir! ✨"
```

**Quand quelqu'un envoie DM sans contexte** :
```
"Salaam! 💌

Merci pour ton message!

C'est pour une info spécifique ? Un conseil style ?
Une question délai ? Je suis toute ouïes.

Dis-moi comment on peut t'aider. 😊"
```

**Quand quelqu'un se trompe de numero/DM** :
```
"Salaam,

Je crois tu nous as tapé un message par erreur?
Ou tu cherchais notre WhatsApp ?

Si c'est pour Ôriya :
📲 WhatsApp: [numéro]
📧 Messages Facebook : Je suis là 24/7

Pas de souci, reste tu! 🤭"
```

---

### MODERATION POLICY

**À ACCEPTER** :
- Questions honnêtes (même critiques)
- Feedback négatif sincère
- Jalousie/Haters (ignore & mute)
- Questions random (répondre avec humour)

**À SUPPRIMER** :
- Spam (autres produits/services)
- Harcèlement sexiste/raciste
- Faux avis (de concurrents)
- Contenu non-pertinent x3

**Ton pour haters** :
```
Si quelqu'un dit : "C'est too expensive!"

Réponse : "Salaam! On comprend. Ôriya c'est luxury
pour la majorité des Marocaines (pas pour tous).

Si tu veux des bijoux moins cher, y'a d'autres options!
Pas de problème. 🤗"

NE PAS insulter. NE PAS défendre aggressively.
Juste cool & confident.
```

---

## DASHBOARD & TRACKING

### METRICS À SUIVRE (DAILY)

```
📊 Facebook Page Metrics :
- Followers (new follows/day)
- Engagement rate (comments + reactions / reach)
- Click-through rate (CTR)
- Cost per click (CPC)
- Reach (organic vs paid)

📲 WhatsApp Metrics :
- Incoming inquiries (daily count)
- Response time (avg minutes)
- Conversion rate (inquiry → purchase)
- Customer sentiment (positive/neutral/negative)

💰 Business Metrics :
- Revenue from Facebook ads
- Revenue from organic
- Average order value (AOV)
- Customer acquisition cost (CAC)
- Return on ad spend (ROAS)

⏰ Content Metrics :
- Post frequency (posts/week)
- Best performing content type
- Best posting times
- Audience growth week-over-week
```

### TOOLS RECOMMANDÉS

```
✅ Facebook Ads Manager (gratuit, essential)
✅ Meta Business Suite (gratuit, essential)
✅ WhatsApp Business API (avec Twilio ou Interakt)
✅ Google Sheets (gratuit, pour tracking manuel)
✅ Canva Pro (5000 MAD/an, pour créatives)
✅ Later ou Buffer (optionnel, scheduling)
```

---

## FINAL CHECKLIST

Avant de lancer ta stratégie Ôriya Facebook:

- [ ] Page Facebook crée avec infos complètes
- [ ] Images couverture uploadées (2 versions)
- [ ] WhatsApp number linkée au CTA
- [ ] 10 posts calendrier préparer (2 semaines)
- [ ] Carousels (3) ready to post
- [ ] Video (3) filmées & editées
- [ ] Facebook Ads campaign set up
- [ ] Budget alloué & payment method confirmed
- [ ] FAQ printed/ready pour team
- [ ] Response templates saved
- [ ] Team trained sur tone of voice
- [ ] Analytics dashboard setup

---

## CONCLUSION

Ôriya Maroc, c'est plus qu'une page Facebook.
C'est une communauté de femmes marocaines qui refusent
de choisir entre qualité et accessibilité.

Ton rôle : Être présente, authentique, et toujours
du côté de ta cliente.

Le reste ? Les ventes, les likes, les followers...
c'est juste les conséquences.

Bon courage. On est avec toi. ✨

*Écrit avec amour pour Ôriya Maroc*
*Stratégie pour 2026*
