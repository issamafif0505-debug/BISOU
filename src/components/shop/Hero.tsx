 ...
 *
 * Server Component — les animations sont gérées par <HeroAnimations /> (client island).
 * Les classes `hero-*` servent de sélecteurs GSAP (voir HeroAnimations.tsx).
 */
    ...
    >
      ...
      <Container className="relative z-10">
        <div className="mx-auto flex max-w-3xl flex-col items-center text-center relative">
          ...
          {/* Eyebrow — classe GSAP hero-eyebrow */}
          <div className="hero-eyebrow mb-6 inline-flex items-center gap-3 text-xs uppercase tracking-[0.3em] text-gold/80 font-sans">
            <span className="h-px w-8 bg-gold/60" />
            {eyebrow}
            <span className="h-px w-8 bg-gold/60" />
          </div>

          {/* Titre — classes GSAP hero-title-top / hero-title-bottom */}
          <h1 className="font-serif leading-[1.05] tracking-tight">
            <span className="hero-title-top block text-[clamp(3rem,8vw,6rem)] font-light text-cream" style={{textShadow: '0 0 40px rgba(253,246,227,0.25), 0 2px 8px rgba(0,0,0,0.8)'}}>
              {titleTop}
            </span>
            <span className="hero-title-bottom block text-[clamp(3.2rem,9vw,7rem)] font-light italic text-gold-gradient mt-1">
              {titleBottom}
            </span>
          </h1>

          {/* Sous-titre — classe GSAP hero-subtitle */}
          <p className="hero-subtitle mt-6 max-w-xl text-base md:text-lg text-grey font-sans leading-relaxed">
            {subtitle}
          </p>

          {/* Badge location — classe GSAP hero-location */}
          <div className="hero-location mt-5 inline-flex items-center gap-2 rounded-full border border-gold/30 bg-black-soft/80 px-4 py-2 text-xs text-cream font-sans backdrop-blur">
          ...
          {/* CTAs — classe GSAP hero-cta sur chaque bouton */}
        ...
        {/* Scroll indicator — classe GSAP hero-scroll-indicator */}
        <div
          aria-hidden="true"
          className="hero-scroll-indicator absolute left-1/2 bottom-10 -translate-x-1/2 flex flex-col items-center gap-2 text-[10px] uppercase tracking-[0.3em] text-gold/70 font-sans"
        >
      ...
      {/* Client island — animations GSAP (aucun markup rendu, zéro layout shift) */}
      <HeroAnimations />
    </section>
  );
}
